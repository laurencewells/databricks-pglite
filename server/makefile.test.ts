...
describe("Makefile deployment workflow", () => {
  it("does not execute Databricks commands during a deploy-run dry-run", async () => {
    ...
    const result = spawnSync(
      "make",
      ["-n", "deploy-run", "PROFILE=ps", "TARGET=dev"],
      {
        cwd: repositoryRoot,
        encoding: "utf8",
        env: {
    ...
    expect(result.stdout).toContain("databricks bundle deploy -t dev -p ps");
    expect(result.stdout).toContain("databricks bundle run pglite_app -t dev -p ps");
  });
});
